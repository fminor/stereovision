...
int main(){
	...
	Mat oldPoints[2], newPoints[2];
	bool patternFound; vector<Point2f> corners[2];
	corners[0] = cornersFromImage(path, "%sL%02d.bmp", pair, boardSize, patternFound);
	corners[1] = cornersFromImage(path, "%sR%02d.bmp", pair, boardSize, patternFound);
	vector<Point2f> vpoints[2]; int points[6] = { 17, 42, 67, 9 };
	for (int i = 0; i < 2; i++){
		for (int k = 0; k < 4; k++){
			vpoints[i].push_back(corners[i][points[k]]);
		}
		
	}
	for (int i = 0; i < 2; i++){
		oldPoints[i] = Mat(vpoints[i]);
		undistortPoints(oldPoints[i], newPoints[i], cameraMatrix[i], distCoeffs[i], R_[i], P[i]);
		cout << "oldPoints" << i << ": " << oldPoints[i] << endl;
		cout << "newPoints" << i << ": " << newPoints[i] << endl;
	}
	...
	for (int i = 0; i < 2; i++){
		...
		for (int k = 0; k < 4; k++){
			circle(cimg[i], Point(vpoints[i][k].x, vpoints[i][k].y), 4, Scalar(0, 255, 0), 1, 8);
		}
	}
